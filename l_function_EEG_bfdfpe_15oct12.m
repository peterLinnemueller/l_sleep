function ret = l_function_EEG_bfdfpe_15oct12(currDir,workDir)
...
%  =============
  ...
  %  correction done AFTER epoching, to leave the raw spm8-file untouched!
...
%   D=chantype(D,indEEG_ECG,'ECG');
  ...
  D=D.chantype(indEEG_EOGhor,'EOG');
  D=D.chantype(indEEG_EOGver,'EOG');
  D=D.chantype(indEEG_EMG,'EMG');
  D=D.chantype(indEEG_ECG,'ECG');
  ...
  % 8 pure EEG channels only (no EOG,..):
...
% % select oure EEG channels only:
%   for j=1:length(indSleepEEG)
%     channelTypesNew{indSleepEEG(j)} = 'EEG'; 
%   end

  for j=1:length(eegChannels.chanNumber)
    channelTypesNew{eegChannels.chanNumber(j)} = 'EEG'; 
  end

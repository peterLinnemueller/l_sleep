...
s = ['find -L -mindepth 3 -maxdepth 4 -type d  | grep -E ".*subj[0-9].*sleep.*_0[1-2]\.ds$" | sort -n > folderList_subjMegDirs.txt']
system(s);
...
for crun = 1:nrun
    ...
    inputs{2, crun} = cellstr(spm_select('FPList',char([rootDir,folderList{crun}(2:end)]),'^spm8_subj.*.mat$')); % M/EEG Epoching: File Name - cfg_files
    inputs{3, crun} = cellstr(spm_select('FPList',char([rootDir,folderList{crun}(2:end)]),'^trials_.*all.mat$'));% M/EEG Epoching: File Name - cfg_files
    inputs{4, crun} = 0; % M/EEG Epoching: Padding - cfg_entry
end
spm('defaults', 'EEG');
spm_jobman('initcfg');

function ret = l_function_create_trialdef_05nov12(S)
% function ret = l_function_create_trialdef_05nov12(workDir, anaDir, currDir, D, PSI_START_MS, PSI_END_MS)
...
fprintf(['\nexecuting l_function_create_trialdef_26oct12(\n\tworkDir=%s,',...
  '\n\tanaDir=%s,\n\tcurrDir=%s, \n\tD=%s, \n\tPSI_START_MS=%d,',...
  '\n\tPSI_END_MS=%d)\n'], workDir, anaDir, currDir, D.fname, PSI_START_MS, PSI_END_MS);
fprintf('\n...moving to %s\n',pwd);
fprintf('\nstart processing...\n');
...
%   saveToFile = [D.path,'/trials_spindels_psi',num2str(PSI_START_MS),'_',num2str(PSI_END_MS),'_overlap_',num2str(D.fsample),'Hz_',spindleInfo(i).sensor{1}];  % save to spm8 meg data directory
  saveToFile = [workDir,'/',anaDir,'/trials_spindels_psi',num2str(PSI_START_MS),'_',num2str(PSI_END_MS),'_overlap_',num2str(D.fsample),'Hz_',spindleInfo(i).sensor{1}];  % save to spm8 meg data directory
...
saveToFile = [workDir,'/',anaDir,'/trials_spindels_psi',num2str(PSI_START_MS),'_',num2str(PSI_END_MS),'_nonoverlap_',num2str(D.fsample),'Hz_',spindleInfo(i).sensor{1}];  % save to spm8 meg data directory
  save(saveToFile, 'trl', 'conditionlabels');
  fprintf('...saved to %s.mat\n',saveToFile);
...
%     saveAllToFile = [D.path,'/trials_spindels_all_psi',num2str(PSI_START_MS),'_',num2str(PSI_END_MS),'_',num2str(D.fsample),'Hz_all'];  % save to spm8 meg data directory
    saveAllToFile = [workDir,'/',anaDir,'/trials_spindels_all_psi',num2str(PSI_START_MS),'_',num2str(PSI_END_MS),'_',num2str(D.fsample),'Hz_all'];  % save to spm8 meg data directory
    trl = trlAll;
...
spindleDurSamples = round(spindleDur*samplingFreq); % number of samples per spindle (integer)

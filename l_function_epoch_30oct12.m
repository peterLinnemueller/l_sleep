function ret = l_function_epoch_30oct12(S)
% function ret = l_function_epoch_30oct12(currDir, workDir, DATA_FILE,
...
BC           = S.BC;
 ...
 cd(workDir)
  ...
  fprintf(['\nexecuting l_function_epoch_19oct12(\n\tcurrDir=%s,',...
          '\n\tworkDir=%s,\n\tDATA_FILE=%s, \n\tPSI_START_MS=%d,',...
          '\n\tPSI_END_MS=%d, \n\tCONDITION=%s, \n\tOVERLAP=%d, \n\tBC=%d)\n'],...
          currDir,workDir,DATA_FILE, PSI_START_MS, PSI_END_MS, CONDITION,...
          OVERLAP,BC);
  fprintf('\n...moving to %s\n',pwd);
  fprintf('\nstart processing...\n');
  ...
  tic
  ret = l_function_create_trialdef_30oct12(S);  
  toc
...
epochinfo=load(spm_select('FPList',workDir,['^trials_spindels_psi',num2str(PSI_START_MS),'_',num2str(PSI_END_MS),'_',sOVERLAP,'_',num2str(D.fsample),'Hz_',CONDITION,'.mat$'])); 
